...
const AlumniSearchPage = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="mb-7">
        <h1 className="text-2xl font-bold text-zinc-900">Find Students to Mentor</h1>
        <p className="text-zinc-400 text-sm mt-0.5">Discover students by skills and offer your guidance</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 p-5 mb-6">
        ...
        <div className="flex gap-3">
          <div className="flex-1">
            <SkillSelector selected={skills} onChange={setSkills} placeholder="React.js, Machine Learning, Python..." />
          </div>
          <button
            onClick={() => doSearch(skills)}
            className="flex items-center gap-2 px-5 py-2.5 bg-violet-600 text-white font-semibold rounded-xl hover:bg-violet-700 transition-colors flex-shrink-0 self-end text-sm"
          >
            <Search className="h-4 w-4" /> Search
          </button>
        </div>
        {skills.length > 0 && (
          <button onClick={() => { setSkills([]); doSearch([]); }} className="text-xs text-zinc-400 hover:text-red-500 mt-3 transition-colors">
            Clear filters
          </button>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-zinc-400 mt-1">Try different skills</p>
        </div>
      ) : (
        <>
          {total > 0 && (
